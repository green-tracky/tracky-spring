...
public class RSAUtil {
    ...
    // 캐시된 공개키 저장소
    private static JwtKeySet cachedKeySet = null;
    ...
    /**
     * 카카오의 OIDC 공개키 목록을 캐싱하여 반환합니다.
     * 캐시가 만료되었거나 없으면 네트워크에서 다시 다운로드합니다.
     *
     * @return JwtKeySet - 카카오 공개키 목록
     */
    private static JwtKeySet getCachedRSAKeySet() {
        long now = Instant.now().getEpochSecond();

        // 캐시가 없거나 만료되었으면 다시 다운로드
        if (cachedKeySet == null || (now - lastFetchedTime) > CACHE_EXPIRATION_TIME) {
            log.info("공개키 캐시가 없거나 만료되어 새로운 공개키를 다운로드합니다.");
            cachedKeySet = downloadRSAKey();
            lastFetchedTime = now;
        } else {
            log.info("캐시된 공개키를 사용합니다.");
        }
     ...
     */
    private static RSASSAVerifier createRSAVerifier(JwtKeySet keySet) throws JOSEException {
        // 2. ID 토큰을 서명한 공개키를 JWKS에서 찾습니다.
        //    여기서는 두 번째 키(index 1)를 사용하도록 하드코딩 되어 있습니다.
        //    (더 견고한 구현을 위해서는 토큰의 헤더(kid)와 일치하는 키를 동적으로 찾아야 합니다.)
        ...
        // 3. n, e 값을 사용하여 RSA 공개키 객체를 생성합니다.
        //    n, e 값은 Base64URL로 인코딩되어 있으므로, 디코딩하여 BigInteger로 변환합니다.
    ...
    /**
     * 카카오의 OIDC(OpenID Connect) 공개키 목록을 다운로드합니다.
     * 이 공개키들은 카카오가 서명한 ID 토큰의 진위 여부를 검증하는 데 사용됩니다.
     *
     * @return JwtKeySet - 카카오의 공개키 목록을 담은 객체
     */
    private static JwtKeySet downloadRSAKey() {
        ...
        // RestTemplate이 내부적으로 Jackson 라이브러리를 사용하여 JSON을 객체로 매핑합니다.
    ...
    public static OAuthProfile verify(String idToken) {
        ...
        try {
            // 4. ID 토큰 파싱
            //    문자열 형태의 ID 토큰을 SignedJWT 객체로 파싱하여 헤더, 페이로드, 서명 부분을 분리합니다. base64 디코딩
            SignedJWT signedJWT = SignedJWT.parse(idToken);

            // 5. 서명 검증 준비
            //    생성한 RSA 공개키를 이용하여 서명 검증기(RSASSAVerifier)를 생성합니다.
            ...
            if (isVerified) {
                ...
                // 7. 페이로드(Payload) 추출
                //    검증이 성공하면, 토큰의 페이로드 부분을 문자열(JSON 형식)로 가져옵니다.
                String payload = signedJWT.getPayload().toString();
                log.debug("페이로드 : " + payload);

                // 8. 페이로드를 객체로 변환
                //    Jackson의 ObjectMapper를 사용하여 JSON 문자열을 OAuthProfile 객체로 변환합니다.
                ObjectMapper objectMapper = new ObjectMapper();
                OAuthProfile profile = objectMapper.readValue(payload, OAuthProfile.class);
                return profile; // 사용자 정보가 담긴 객체를 반환합니다.

            } else {
                // 서명 검증에 실패한 경우, 예외를 발생시킵니다.
